...
$cek = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_user where siswa_id_siswa='$siswa_id_siswa'"));
if ($cek > 0) {
...
window.location.href='datauser.php';</script>";
    ...
    if (empty($nama)) {
        ...
        $query = mysqli_query($conn, "INSERT INTO tb_user (username,password,level,siswa_id_siswa)values('$nama','$password','$level','$siswa_id_siswa');");
        if ($query) {
