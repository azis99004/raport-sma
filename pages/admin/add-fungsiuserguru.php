...
$cek = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_user where kode_guru='$kode_guru'"));
if ($cek > 0) {
...
window.location.href='datauser.php';</script>";
    ...
    if (empty($nama)) {
        ...
        $query = mysqli_query($conn, "INSERT INTO tb_user (username,password,level,kode_guru)values('$nama','$password','$level','$kode_guru');");
        if ($query) {
