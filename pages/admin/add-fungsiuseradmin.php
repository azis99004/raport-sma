...
$cek = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_user where id_admin='$id_admin'"));
if ($cek > 0) {
...
window.location.href='datauser.php';</script>";
    ...
    if (empty($nama)) {
        ...
        $query = mysqli_query($conn, "INSERT INTO tb_user (username,password,level,id_admin)values('$nama','$password','$level','$id_admin');");
        if ($query) {
