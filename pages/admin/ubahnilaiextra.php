...
include "../koneksi.php";
if (isset($_GET['id'])) {
    $tampilkelassis = mysqli_query($conn, "SELECT * FROM tb_extrakulikuler WHERE kode_nilai_extra = '$_GET[id]'");
    $kelassis = mysqli_fetch_assoc($tampilkelassis);
}
?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Ubah Data Nilai Extrakulikuler Siswa</h1>
                    <p class="mb-4">Silahkan ubah data nilai extrakulikuler siswa yang ingin diubah!

                        <!-- DataTales Example -->
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Data Nilai Extrakulikuler</h6>
                        </div>
                        <div class="container">
                            <h2>Ubah Data</h2>
                            <p>Silahkan jika kalian ingin mengubah data, lakukan dengan bijak!</p>
                            <form action="ubah-fungsinilaiextra.php" method="POST" class="needs-validation" novalidate>
                                <div class="form-group">
                                    <input type="hidden" name="kode_nilai_extra" value="<?= $kelassis["kode_nilai_extra"]; ?>">
                                </div>
                                <div class="form-group">
                                    <label for="sel1" readonly>Nama Siswa:</label>
                                    <select class="form-control" id="sel1" name="id_siswa" required>
                                        <?php
                                        $tampil = mysqli_query($conn, "SELECT * FROM tb_siswa ORDER BY id_siswa");
                                        while ($w = mysqli_fetch_array($tampil)) {
                                            if ($kelassis['id_siswa'] == $w['id_siswa']) {
                                                echo "<option value=$w[id_siswa] selected>$w[nama_siswa]</option>";
                                            } else {
                                                echo "<option value=$w[id_siswa]>$w[nama_siswa]</option>";
                                            }
                                        }
                                        ?>
                                    </select>
                                    <div class="form-group">
                                        <label for="kegiatan_ekstrakulikuler">Kegiatan Ekstrakulikuler :</label>
                                        <input type="text" class="form-control" id="deskripsi" placeholder="Isikan Keterangan Ekstrakulikuler" name="kegiatan_ekstrakulikuler" required>
                                        <div class="valid-feedback">Ok</div>
                                        <div class="invalid-feedback">Isi Nama Kegiatan Ekstrakulikuler</div>
                                    </div>
                                    <div class="form-group">
                                        <label for="predikat">Predikat :</label>
                                        <input type="text" class="form-control" id="deskripsi" placeholder="Isikan deskripsi" name="predikat" required>
                                        <div class="valid-feedback">Ok</div>
                                        <div class="invalid-feedback">Isi predikat Siswa</div>
                                    </div>
                                    <div class="form-group">
                                        <label for="keterangan">Keterangan :</label>
                                        <input type="text" class="form-control" id="keterangan" placeholder="Isikan keterangan" name="keterangan" required>
                                        <div class="valid-feedback">Ok</div>
